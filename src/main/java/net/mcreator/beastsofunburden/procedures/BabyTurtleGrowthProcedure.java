...
public class BabyTurtleGrowthProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity instanceof BabyTurtleVillagerEntity _datEntI ? _datEntI.getEntityData().get(BabyTurtleVillagerEntity.DATA_timer) : 0) < 201) {
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = new TurtleVillagerEntity(BouModEntities.TURTLE_VILLAGER.get(), _level);
				entityToSpawn.moveTo(x, y, z, world.getRandom().nextFloat() * 360F, 0);
				if (entityToSpawn instanceof Mob _mobToSpawn)
					_mobToSpawn.finalizeSpawn(_level, _level.getCurrentDifficultyAt(entityToSpawn.blockPosition()), MobSpawnType.MOB_SUMMONED, null, null);
				_level.addFreshEntity(entityToSpawn);
			}
			if ((findEntityInWorldRange(world, TurtleVillagerEntity.class, x, y, z, 1)) instanceof TurtleVillagerEntity _datEntSetI)
				_datEntSetI.getEntityData().set(TurtleVillagerEntity.DATA_variant, (int) (entity instanceof BabyTurtleVillagerEntity _datEntI ? _datEntI.getEntityData().get(BabyTurtleVillagerEntity.DATA_variant) : 0));
