...
public class BabySheepGrowthProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity instanceof BabySheepVillagerEntity _datEntI ? _datEntI.getEntityData().get(BabySheepVillagerEntity.DATA_timer) : 0) < 2000) {
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = new SheepVillagerEntity(BouModEntities.SHEEP_VILLAGER.get(), _level);
				entityToSpawn.moveTo(x, y, z, world.getRandom().nextFloat() * 360F, 0);
				if (entityToSpawn instanceof Mob _mobToSpawn)
					_mobToSpawn.finalizeSpawn(_level, _level.getCurrentDifficultyAt(entityToSpawn.blockPosition()), MobSpawnType.MOB_SUMMONED, null, null);
				_level.addFreshEntity(entityToSpawn);
			}
			if ((findEntityInWorldRange(world, SheepVillagerEntity.class, x, y, z, 1)) instanceof SheepVillagerEntity _datEntSetI)
				_datEntSetI.getEntityData().set(SheepVillagerEntity.DATA_variant, (int) (entity instanceof BabySheepVillagerEntity _datEntI ? _datEntI.getEntityData().get(BabySheepVillagerEntity.DATA_variant) : 0));
