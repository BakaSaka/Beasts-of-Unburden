...
public class BabyWolfGrowthProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity instanceof BabyWolfVillagerEntity _datEntI ? _datEntI.getEntityData().get(BabyWolfVillagerEntity.DATA_timer) : 0) < 2000) {
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = new WolfVillagerEntity(BouModEntities.WOLF_VILLAGER.get(), _level);
				entityToSpawn.moveTo(x, y, z, world.getRandom().nextFloat() * 360F, 0);
				if (entityToSpawn instanceof Mob _mobToSpawn)
					_mobToSpawn.finalizeSpawn(_level, _level.getCurrentDifficultyAt(entityToSpawn.blockPosition()), MobSpawnType.MOB_SUMMONED, null, null);
				_level.addFreshEntity(entityToSpawn);
			}
			if ((findEntityInWorldRange(world, WolfVillagerEntity.class, x, y, z, 1)) instanceof WolfVillagerEntity _datEntSetI)
				_datEntSetI.getEntityData().set(WolfVillagerEntity.DATA_variant, (int) (entity instanceof BabyWolfVillagerEntity _datEntI ? _datEntI.getEntityData().get(BabyWolfVillagerEntity.DATA_variant) : 0));
