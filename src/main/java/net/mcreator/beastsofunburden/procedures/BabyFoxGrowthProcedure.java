...
public class BabyFoxGrowthProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity instanceof BabyFoxVillagerEntity _datEntI ? _datEntI.getEntityData().get(BabyFoxVillagerEntity.DATA_timer) : 0) < 2000) {
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = new FoxVillagerEntity(BouModEntities.FOX_VILLAGER.get(), _level);
				entityToSpawn.moveTo(x, y, z, world.getRandom().nextFloat() * 360F, 0);
				if (entityToSpawn instanceof Mob _mobToSpawn)
					_mobToSpawn.finalizeSpawn(_level, _level.getCurrentDifficultyAt(entityToSpawn.blockPosition()), MobSpawnType.MOB_SUMMONED, null, null);
				_level.addFreshEntity(entityToSpawn);
			}
			if ((findEntityInWorldRange(world, FoxVillagerEntity.class, x, y, z, 1)) instanceof FoxVillagerEntity _datEntSetI)
				_datEntSetI.getEntityData().set(FoxVillagerEntity.DATA_variant, (int) (entity instanceof BabyFoxVillagerEntity _datEntI ? _datEntI.getEntityData().get(BabyFoxVillagerEntity.DATA_variant) : 0));
