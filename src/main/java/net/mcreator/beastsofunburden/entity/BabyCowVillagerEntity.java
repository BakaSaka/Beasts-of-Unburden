...
public class BabyCowVillagerEntity extends Animal {
	public static final EntityDataAccessor<Integer> DATA_variant = SynchedEntityData.defineId(BabyCowVillagerEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_timer = SynchedEntityData.defineId(BabyCowVillagerEntity.class, EntityDataSerializers.INT);
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.BABY_COW_VILLAGER.get(), 20, 1, 2));
	}

	public BabyCowVillagerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.BABY_COW_VILLAGER.get(), world);
	}

	public BabyCowVillagerEntity(EntityType<BabyCowVillagerEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		BabyCowVariationSpawnProcedure.execute(this);
		return retval;
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		BabyCowVillagerEntity retval = BouModEntities.BABY_COW_VILLAGER.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(BouModEntities.BABY_COW_VILLAGER.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
