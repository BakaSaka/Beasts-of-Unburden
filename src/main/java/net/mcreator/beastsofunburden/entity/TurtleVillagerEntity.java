...
import net.mcreator.beastsofunburden.procedures.TurtleVariationSpawnProcedure;
...
public class TurtleVillagerEntity extends Animal {
	public static final EntityDataAccessor<Integer> DATA_variant = SynchedEntityData.defineId(TurtleVillagerEntity.class, EntityDataSerializers.INT);
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.TURTLE_VILLAGER.get(), 20, 2, 4));
	}

	public TurtleVillagerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.TURTLE_VILLAGER.get(), world);
	}

	public TurtleVillagerEntity(EntityType<TurtleVillagerEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		TurtleVariationSpawnProcedure.execute(this);
		return retval;
	}
		...
		if (sourceentity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return new TextComponent("Turtle Villager");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					packetBuffer.writeVarInt(TurtleVillagerEntity.this.getId());
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		TurtleVillagerEntity retval = BouModEntities.TURTLE_VILLAGER.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return Ingredient.of(new ItemStack(Blocks.SEAGRASS)).test(stack);
	}

	public static void init() {
		SpawnPlacements.register(BouModEntities.TURTLE_VILLAGER.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
