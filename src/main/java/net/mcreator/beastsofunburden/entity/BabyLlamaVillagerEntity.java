...
import net.mcreator.beastsofunburden.procedures.BabyLlamaVariationSpawnProcedure;
...
public class BabyLlamaVillagerEntity extends Animal {
	public static final EntityDataAccessor<Integer> DATA_variant = SynchedEntityData.defineId(BabyLlamaVillagerEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_timer = SynchedEntityData.defineId(BabyLlamaVillagerEntity.class, EntityDataSerializers.INT);
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.BABY_LLAMA_VILLAGER.get(), 20, 1, 2));
	}

	public BabyLlamaVillagerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.BABY_LLAMA_VILLAGER.get(), world);
	}

	public BabyLlamaVillagerEntity(EntityType<BabyLlamaVillagerEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		BabyLlamaVariationSpawnProcedure.execute(this);
		return retval;
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		BabyLlamaVillagerEntity retval = BouModEntities.BABY_LLAMA_VILLAGER.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(BouModEntities.BABY_LLAMA_VILLAGER.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
