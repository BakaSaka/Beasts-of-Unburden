...
public class TopAnimalChestEntityEntity extends PathfinderMob {
	public static final EntityDataAccessor<Integer> DATA_variant = SynchedEntityData.defineId(TopAnimalChestEntityEntity.class, EntityDataSerializers.INT);
	private static final Set<ResourceLocation> SPAWN_BIOMES = null;

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.TOP_ANIMAL_CHEST_ENTITY.get(), 20, 4, 4));
	}

	public TopAnimalChestEntityEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.TOP_ANIMAL_CHEST_ENTITY.get(), world);
	}

	public TopAnimalChestEntityEntity(EntityType<TopAnimalChestEntityEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.1f;
		xpReward = 0;
		setNoAi(true);
	...
	public static void init() {
		SpawnPlacements.register(BouModEntities.TOP_ANIMAL_CHEST_ENTITY.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
