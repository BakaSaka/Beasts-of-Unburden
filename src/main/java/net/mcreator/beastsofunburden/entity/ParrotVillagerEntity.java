...
public class ParrotVillagerEntity extends Animal {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.PARROT_VILLAGER.get(), 20, 2, 4));
	}

	public ParrotVillagerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.PARROT_VILLAGER.get(), world);
	}

	public ParrotVillagerEntity(EntityType<ParrotVillagerEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		ParrotVillagerEntity retval = BouModEntities.PARROT_VILLAGER.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(BouModEntities.PARROT_VILLAGER.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
