...
import net.mcreator.beastsofunburden.procedures.SpiderVariationSpawnProcedure;
...
public class SpiderVillagerEntity extends Animal {
	public static final EntityDataAccessor<Integer> DATA_variant = SynchedEntityData.defineId(SpiderVillagerEntity.class, EntityDataSerializers.INT);
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.SPIDER_VILLAGER.get(), 20, 2, 4));
	}

	public SpiderVillagerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.SPIDER_VILLAGER.get(), world);
	}

	public SpiderVillagerEntity(EntityType<SpiderVillagerEntity> type, Level world) {
		super(type, world);
		maxUpStep = 1.5f;
		xpReward = 0;
		setNoAi(false);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		SpiderVariationSpawnProcedure.execute(this);
		return retval;
	}
		...
		if (sourceentity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return new TextComponent("Spider Villager");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					packetBuffer.writeVarInt(SpiderVillagerEntity.this.getId());
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		SpiderVillagerEntity retval = BouModEntities.SPIDER_VILLAGER.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return Ingredient.of(new ItemStack(Items.SPIDER_EYE)).test(stack);
	}

	public static void init() {
		SpawnPlacements.register(BouModEntities.SPIDER_VILLAGER.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
