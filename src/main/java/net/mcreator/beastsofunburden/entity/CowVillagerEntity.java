...
import net.mcreator.beastsofunburden.procedures.CowVariationSpawnProcedure;
...
public class CowVillagerEntity extends Animal {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.COW_VILLAGER.get(), 20, 2, 4));
	}

	public CowVillagerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.COW_VILLAGER.get(), world);
	}

	public CowVillagerEntity(EntityType<CowVillagerEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		CowVariationSpawnProcedure.execute(this);
		return retval;
	}
		...
		if (sourceentity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return new TextComponent("Cow Villager");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					packetBuffer.writeVarInt(CowVillagerEntity.this.getId());
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		CowVillagerEntity retval = BouModEntities.COW_VILLAGER.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(BouModEntities.COW_VILLAGER.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
