...
public class RabbitVillagerEntity extends Animal {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.RABBIT_VILLAGER.get(), 20, 2, 4));
	}

	public RabbitVillagerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.RABBIT_VILLAGER.get(), world);
	}

	public RabbitVillagerEntity(EntityType<RabbitVillagerEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
		...
		if (sourceentity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					packetBuffer.writeVarInt(RabbitVillagerEntity.this.getId());
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		RabbitVillagerEntity retval = BouModEntities.RABBIT_VILLAGER.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(BouModEntities.RABBIT_VILLAGER.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
