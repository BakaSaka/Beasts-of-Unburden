...
public class ChickenVillagerEntity extends Animal {
	public static final EntityDataAccessor<Integer> DATA_variant = SynchedEntityData.defineId(ChickenVillagerEntity.class, EntityDataSerializers.INT);
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.CHICKEN_VILLAGER.get(), 20, 2, 4));
	}

	public ChickenVillagerEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.CHICKEN_VILLAGER.get(), world);
	}

	public ChickenVillagerEntity(EntityType<ChickenVillagerEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
		...
		if (sourceentity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return new TextComponent("Chicken Villager");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					packetBuffer.writeVarInt(ChickenVillagerEntity.this.getId());
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		ChickenVillagerEntity retval = BouModEntities.CHICKEN_VILLAGER.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(BouModEntities.CHICKEN_VILLAGER.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
