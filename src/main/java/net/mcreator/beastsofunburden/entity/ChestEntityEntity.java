...
import net.mcreator.beastsofunburden.procedures.ChestOrangeProcedure;
...
public class ChestEntityEntity extends PathfinderMob {
	public static final EntityDataAccessor<String> DATA_vendor = SynchedEntityData.defineId(ChestEntityEntity.class, EntityDataSerializers.STRING);
	private static final Set<ResourceLocation> SPAWN_BIOMES = null;

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MISC).add(new MobSpawnSettings.SpawnerData(BouModEntities.CHEST_ENTITY.get(), 20, 4, 4));
	}

	public ChestEntityEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BouModEntities.CHEST_ENTITY.get(), world);
	}

	public ChestEntityEntity(EntityType<ChestEntityEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.1f;
		xpReward = 0;
		setNoAi(true);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		ChestOrangeProcedure.execute(this);
		return retval;
	}
	...
	public static void init() {
		SpawnPlacements.register(BouModEntities.CHEST_ENTITY.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
